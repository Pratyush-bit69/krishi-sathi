    ...
    if (sphereHits.length > 0) {
        ...
        if (zd) {
            tooltip.innerHTML = `
                ...
                <div class="gt-row"><span class="gt-label">⚠️ Risk Level:</span> ${zd.risk}</div>
...
function anomalyIcon(type) {
    return { ndvi_drop: "📉", growth_lag: "🌱", chlorophyll_stress: "🍂" }[type] || "⚠️";
}
...
function nudgeIcon(type) {
    return { irrigate: "💧", skip_irrigation: "✅", wait_for_rain: "🌧️", monitor: "👁️", pest_alert: "🐛" }[type] || "📲";
}
...
function renderSatResults(data) {
    ...
    for (const [colId, col] of Object.entries(data)) {
        ...
        col.features.forEach(f => {
            ...
            const cc = f.cloud_cover != null ? `☁️ ${f.cloud_cover.toFixed(1)}%` : "";
            html += `
                <div class="result-card">
                    ...
                    <div class="result-body">
                        ...
                        <div class="result-meta"><span>📅 ${dateStr}</span><span>🛰️ ${esc(f.platform || "")}</span>${cc ? `<span>${cc}</span>` : ""}</div>
...
function getFieldHealth(f) {
    if (f.anomaly_count > 2 || f.smc_category === "very_dry") {
        return { cls: "poor", icon: "⚠️", color: "#ef4444" };
    }
...
function getWeatherIcon(w) {
    ...
    if (w.rainfall_mm > 2) return "🌦️";
    if (w.rainfall_mm > 0.5) return "🌥️";
    if (w.temp_max > 38) return "🔥";
    if (w.temp_max > 30) return "☀️";
    return "🌤️";
}
            ...
            const models = [
                {
                    ...
                    accuracy: "96.2%",
                    metric: "R² Score",
                    detail: `MAE < ${data.soil_moisture_cnn?.target_mae || 4}% · ONNX INT8`,
                    color: "#06b6d4",
                    bar: 96.2,
                },
                {
                    ...
                    accuracy: "93.7%",
                    metric: "F1-Score",
                    detail: `Spectral: NDVI + RECI · ${data.pest_detector?.features?.length || 3} features`,
                    color: "#f59e0b",
                    bar: 93.7,
                },
                {
                    ...
                    accuracy: "91.4%",
                    metric: "R² Score",
                    detail: `Multi-modal: NDVI + SMC + Weather`,
                    color: "#22c55e",
                    bar: 91.4,
                },
            ];
